...
本文保留首次评估和迁移规划；Codex 换肤交接和 Godot 原生导出第一版均已实现，见第 9、10 节及 [Godot 使用说明](godot-native-export.md)。
...
本节记录第一版规划范围；当前已经实现的子集与限制见第 10 节和使用说明。
...
原 Gemini 调用路径保留，共享准备和结果拆分逻辑。交接导入生成 Spine 皮肤与图集，
之后可使用第 10 节的 Godot 导出。使用步骤与接口见
...
第一版已实现。新增独立 CLI、应用 `POST /api/export/godot` 接口和 Export 格式选择，
产出可运行的原生骨骼／切片场景、动画资源、皮肤辅助脚本、通用角色描述与导出报告。
原生工程可脱离 Spine runtime 运行；应用内部预览仍使用原 Spine runtime。

采用 Godot 文本资源输出，导出时不要求安装 Godot，验收时使用本机 Godot 4.7.2。
普通骨骼变换、静态层级、皮肤和六套基础动作已通过实际引擎数值对照；
不支持的高级特性在写出工程前拒绝并列出。贝塞尔默认采样 60 Hz，保留阶跃语义，
每套动画包含未动画属性的初始值，避免动作切换时残留姿势。

同时修正了 `examples/sombrero/skeleton.json` 遗留的旧动画格式：此前声明 4.2，
却仍包含 `angle` 和旧曲线。此次用已有生成器的 `_to_spine_42` 修正动画，
没有更换其骨架或图集。

完整入口、范围和验证方法见 [Godot 原生导出说明](godot-native-export.md)。
上面的首次评估和阶段计划保留为设计背景，以本节和使用说明为当前实现状态。

验证：36 项测试通过，包括实际 Godot 与项目中官方 Spine runtime 的骨骼矩阵对照、
镜像／非等比缩放、跨动作姿态恢复、皮肤切换和应用导出接口；前端构建通过。
通过浏览器完成 Codex 皮肤 → Godot 导出，并用 GPU 渲染生成 27 张检查帧，
覆盖六动作示例及美术角色的原皮肤／导入皮肤。未执行实际 AI 图像生成。
